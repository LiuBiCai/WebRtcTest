import xCloudApiClient, { StartStreamReponse } from "./webrtc"
...
export default class Stream {
    ...
    private _waitInterval: NodeJS.Timeout | undefined
    ...
    refreshState(){
        return new Promise<string>((resolve, reject) => {
            console.log('this.getSessionPath() '+this.getSessionPath())
    ...
    sendChatSDPOffer(sdpOffer: { sdp: any }){
        return new Promise((resolve, reject) => {
            const body = JSON.stringify({
                'messageType':'offer',
                'requestId': 2,
